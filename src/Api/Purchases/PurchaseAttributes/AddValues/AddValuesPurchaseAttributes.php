<?php

declare(strict_types=1);

namespace Studio15\SailPlay\SDK\Api\Purchases\PurchaseAttributes\AddValues;

use Studio15\SailPlay\SDK\Infrastructure\ApiHttpClient;
use Studio15\SailPlay\SDK\Infrastructure\Error\ApiErrorException;
use Throwable;

/**
 * @see https://ru.sailplay.dev/reference/purchases-purchase-attributes-values-add
 */
final class AddValuesPurchaseAttributes
{
    private const RESOURCE_PATH = 'purchases/purchase-attributes/values/add';

    /**
     * @var ApiHttpClient
     */
    private $apiClient;

    public function __construct(ApiHttpClient $client)
    {
        $this->apiClient = $client;
    }

    /**
     * @throws ApiErrorException
     * @throws Throwable
     */
    public function __invoke(
        AddValuesPurchaseAttributesRequest $addValuesRequest,
        string $token
    ): AddValuesPurchaseAttributesResponse {
        return $this->apiClient->post(
            self::RESOURCE_PATH,
            AddValuesPurchaseAttributesResponse::class,
            $addValuesRequest,
            $token
        );
    }
}
